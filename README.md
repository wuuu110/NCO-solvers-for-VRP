...
This repository provides an up-to-date list of studies addressing Vehicle Routing Problems (VRPs) using Neural Combinatorial Optimization (NCO) solvers. It follows the taxonomy provided in our paper "[Neural Combinatorial Optimization Solvers for Vehicle Routing Problems: A Survey]()". We organize the references in ascending order according to their publication years. Within the same publication year, we arrange the references in ascending order based on the initial letter of each title.
